...
def evaluation_one_frame(mesh_gt, mesh_ours, gt_cam_matrix, our_cam_matrix_path):
    ...
    verts_ours = obj_to_cam(verts_ours, Rmat_ours.float().to(DEVICE), Tmat_ours.float().to(DEVICE))
    verts_gt = obj_to_cam(verts_gt, Rmat_gt, Tmat_gt)
...
def evaluate(gt_mesh_dir, our_dir, gt_cam_matrix_path, seqname, cam_ext, prefix, zfill, logger):
    ...
    if seqname in 'a-hands' or seqname in 'a-eagle':
    ...
    for idx in tqdm(range(len(gt_meshes))):
        ...
        try: 
            ...
            print("our mesh total # num = ", idx + 1)
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument('--our_dir', type=str, default='logdir/seed0/T_swing-e120-b256-seed0-ft2')
    ...
    logger = get_logger(args.seqname, args.our_dir)

    evaluate(args.gt_mesh_dir, args.our_dir, args.gt_cam_path, args.seqname, args.cam_ext, args.type_prefix, args.zfill, logger)
